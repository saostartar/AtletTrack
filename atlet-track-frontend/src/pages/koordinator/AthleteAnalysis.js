...
const AthleteAnalysis = () => {
  ...
  const fetchAthleteAnalysis = async () => {
    try {
      ...
      setError(
        error.response?.data?.message || "Gagal mengambil data analisis atlet"
      );
    } finally {
      setLoading(false);
    }
            ...
            <Col md={7}>
              ...
              <div className="d-flex align-items-center mb-3">
                ...
                <div className="d-flex align-items-center">
                  <div
                    className="circle-score me-2"
                    style={{
                      backgroundColor: `var(--bs-${getScoreVariant(
                        analysis.overallWeightedScore
                      )})`,
                    }}>
                  ...
                  <div>
                    ...
                    <div className="fw-bold">
                      {getPerformanceStatus(analysis.overallWeightedScore)}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <Col md={5}>
              <Card className="border-0 shadow-sm h-100 stat-card">
                <Card.Body className="d-flex align-items-center">
                  <div
                    className={`stat-icon bg-${getScoreVariant(
                      analysis.overallWeightedScore
                    )} bg-opacity-10 text-${getScoreVariant(
                      analysis.overallWeightedScore
                    )}`}>
                  ...
                  <div className="ms-3">
                    ...
                    <h3 className="mb-0 fw-bold">
                      {analysis.overallWeightedScore.toFixed(1)}
                    </h3>
                  </div>
                </Card.Body>
              </Card>
              ...
              <Card.Body className="p-0">
                <div className="table-responsive">
                  <Table hover className="align-middle mb-0 custom-table">
                    ...
                    <tbody>
                      {Object.entries(analysis.evaluationsByType).map(
                        ([type, data]) => (
                          <tr key={type}>
                            <td className="fw-bold text-primary">{type}</td>
                            <td>
                              <Badge
                                bg={getScoreVariant(data.averageScore)}
                            ...
                            <td>
                              <Badge
                                bg={getScoreVariant(data.latestScore)}
                                className="score-badge">
                                {data.latestScore}
                              </Badge>
                            </td>
                ...
                {analysis.strengths.length > 0 ? (
                  <div className="strength-list">
                    {analysis.strengths.map((strength, index) => (
                      <div
                        key={index}
                        className="strength-item mb-3 p-3 bg-success bg-opacity-10 rounded-3">
                  ...
                  <div className="text-center py-4">
                    <p className="text-muted">
                      Tidak ada kekuatan signifikan yang teridentifikasi.
                    </p>
                  </div>
                )}
              </Card.Body>
                ...
                {analysis.weaknesses.length > 0 ? (
                  <div className="weakness-list">
                    {analysis.weaknesses.map((weakness, index) => (
                      <div
                        key={index}
                        className="weakness-item mb-3 p-3 bg-danger bg-opacity-10 rounded-3">
                        <div className="d-flex justify-content-between align-items-center mb-2">
            ...
            {analysis.recommendations.length > 0 ? (
              <div className="recommendation-list">
                {analysis.recommendations.map((recommendation, index) => (
                  <div
                    key={index}
                    className="recommendation-item p-3 mb-3 bg-light rounded-3 border-start border-warning border-4">
                        ...
                        {data.evaluations.length > 0 ? (
                              ...
                              <tbody>
                                {data.evaluations.map((evaluation) => (
                                  <tr key={evaluation.id}>
                                    ...
                                    <td>
                                      <Badge
                                        bg={getScoreVariant(evaluation.skor)}
                                        className="score-badge">
                                        {evaluation.skor}
                                      </Badge>
                                    </td>
                          ...
                          <div className="p-4 text-center">
                            <p className="text-muted mb-0">
                              Tidak ada evaluasi untuk kategori ini.
                            </p>
                          </div>
                        )}
                      </div>
